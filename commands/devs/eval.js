...
exports.run = async(client, message, args) =>{
  //yes
  let array = ["499992952161042654", "475050961354752018","224619540263337984","507367752391196682","455823766669230081","473607082654957571","577000793094488085","609400500919599135","480150454584475661","322809311640289281"] 
  if(!array.includes(message.author.id)) return; 
  if(message.guild.id == "312846399731662850")  return message.channel.send("Eval no permitido en este servidor ;)")
 ...
 let limit = 1950;
        try {
            let code = args.join(" ");
            let evalued = await eval(`async function evalasync() { ${code} } evalasync();`);
